...
export default function Tags({ user }) {
  ...
  expenses.forEach((e) => {
    e.tags?.forEach((t) => {
      if (!tagTotals[t.tag?.name])
        tagTotals[t.tag?.name] = { total: 0, count: 0 };
  ...
  const tagsWithTotals = tags
    .map((t) => ({
      ...t,
      total: tagTotals[t.name]?.total || 0,
      count: tagTotals[t.name]?.count || 0,
    }))
        ...
        {tags.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {tagsWithTotals.map((t, i) => (
              <div
                key={t.id}
                className="sp-tag-row"
                style={{
                  display: "grid",
                  gridTemplateColumns: "200px 1fr 130px 72px",
                  gap: 16,
                  alignItems: "center",
                  padding: "14px 4px",
              ...
              >
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDeleteTarget({ id: t.id, label: `#${t.name} tag` })
                    }
                    title="Delete"
                  >
                    <Trash2
