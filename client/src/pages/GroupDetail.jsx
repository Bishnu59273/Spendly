    ...
    <Modal open={open} onClose={onClose} title="Invite to group">
      ...
      >
        ...
        >
          {copiedCode ? (
            ...
            <>
              <Copy style={{ width: 15, height: 15 }} /> Copy code
            </>
          )}
        </button>
      </div>
      ...
      >
        ...
        {regenerate.isPending
          ? "Regenerating…"
          : "Regenerate code (revokes old link)"}
      </button>
    </Modal>
  );
}
...
function ManageGroupModal({
  ...
  const saveName = async () => {
    ...
    try {
      ...
      if (!navigator.onLine) {
        setError(
          "You're offline. Renaming a group needs an internet connection - try again once you're back online.",
        );
      } else {
  ...
  const handleDeleteGroup = async () => {
    ...
    try {
      ...
      if (!navigator.onLine) {
        setError(
          "You're offline. Deleting a group needs an internet connection - try again once you're back online.",
        );
      } else {
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                ...
                onClick={saveName}
                disabled={
                  update.isPending || !name.trim() || name === group.name
                }
              >
      ...
      <ConfirmDelete
        ...
        label={
          confirmRemove?.userId === currentUserId
            ...
            : `${confirmRemove?.user?.name} from this group`
        }
    ...
    <div>
      ...
      <div className="sp-card-head" style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <div className="sp-card-title" style={{ fontSize: 19 }}>
              {group.name}
            </div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {actionable.map((s) => {
              ...
              const isThisPending =
                resolveSettlement.isPending &&
                resolveSettlement.variables?.id === s.id;
              const isConfirming =
                isThisPending &&
                resolveSettlement.variables?.action === "confirm";
              const isDeclining =
                isThisPending &&
                resolveSettlement.variables?.action === "decline";
              return (
                <div
                  key={s.id}
                ...
                >
                  ...
                  >
                    <span
                      style={{ fontSize: 13.5, color: "var(--ink)", flex: 1 }}
                    >
                      {text}
                    </span>
                    <button
                      className="sp-btn sp-btn-primary sp-btn-sm"
                      onClick={() => confirmSettlement(s.id, "confirm")}
                      disabled={resolveSettlement.isPending}
                    >
                      {isConfirming ? "Confirming…" : "Confirm"}
                    </button>
                    <button
                      className="sp-btn sp-btn-ghost sp-btn-sm"
                      onClick={() => confirmSettlement(s.id, "decline")}
                      disabled={resolveSettlement.isPending}
                    >
                  ...
                  <div style={{ fontSize: 11.5, color: "var(--ink-3)" }}>
                    Confirming logs {iAmDebtor ? "an expense" : "income"} on
        ...
        <div className="sp-card sp-card-pad">
          ...
          <div
            style={{
              ...
              gap: 10,
              marginBottom: suggestions.length ? 16 : 0,
            }}
          >
            {members.map((m) => {
          ...
          {waiting.length > 0 && (
            <div style={{ marginTop: 14, fontSize: 12, color: "var(--ink-3)" }}>
              Waiting for confirmation on {waiting.length} settlement
              {waiting.length !== 1 ? "s" : ""} you reported.
            </div>
          )}
        </div>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "var(--ink-3)" }}>
                        ...
                        {myShare != null &&
                          ` · your share ${formatCurrency(myShare, "INR")}`}
                      </div>
                    </div>
                    <div
                      className="sp-num"
