...
function CreateGroupModal({ open, onClose, onCreated }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!navigator.onLine) {
        setError(
          "You're offline. Creating a group needs an internet connection - try again once you're back online.",
        );
      } else {
...
function JoinGroupModal({ open, onClose, onJoined }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Join a group">
      ...
      />
      {error && (
        <div style={{ fontSize: 13, color: "var(--neg)", marginTop: 8 }}>
          {error}
        </div>
      )}
  ...
  return (
    <div>
      <div className="sp-card-head" style={{ marginBottom: 16 }}>
        <div>
          <div className="sp-card-title" style={{ fontSize: 20 }}>
            Groups
          </div>
          <div className="sp-card-sub">
            Split bills with flatmates and partners
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        >
          ...
          >
            Create a group for your flat or with your partner to start splitting
            expenses.
          </div>
