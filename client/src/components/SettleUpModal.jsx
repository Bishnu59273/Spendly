...
export default function SettleUpModal({
  ...
  const upiLink = payeeUpi
    ? buildUpiLink({
        vpa: payeeUpi,
        payeeName: toMember.user.name,
        amount,
        note: "Spendly settle up",
      })
    : null;

  const nameOf = (member, id) =>
    id === currentUserId ? "You" : member?.user?.name || "Someone";
  ...
  const markSettled = async () => {
    ...
    try {
      ...
      if (!navigator.onLine) {
        setError(
          "You're offline. Settling up needs an internet connection - try again once you're back online.",
        );
      } else {
    ...
    <Modal open={open} onClose={onClose} title="Settle up">
      <div style={{ textAlign: "center", padding: "8px 0 18px" }}>
        <div
          ...
          style={{ fontSize: 30, fontWeight: 800, color: "var(--ink)" }}
        >
      ...
      {isDebtor &&
        (payeeUpi ? (
          ...
          >
            {nameOf(toMember, toUserId)} hasn't added a UPI ID yet, so there's
            no pay link to show. Settle up another way, then mark it as paid
            below.
          </div>
        ))}
      ...
      {isDebtor || isCreditor ? (
        ...
        <div
          style={{ fontSize: 13, color: "var(--ink-3)", textAlign: "center" }}
        >
          Waiting on {nameOf(fromMember, fromUserId)} and{" "}
          {nameOf(toMember, toUserId)} to settle this.
        </div>
      )}
      ...
      {(isDebtor || isCreditor) && (
        ...
        >
          {isDebtor
            ? nameOf(toMember, toUserId)
            : nameOf(fromMember, fromUserId)}{" "}
          will need to confirm before this counts toward your balance. Once
          confirmed, it's logged as {isDebtor ? "an expense" : "income"} on your
