...
export default function GroupExpenseForm({
  ...
  const [percents, setPercents] = useState(() => {
    ...
    if (initial?.splitType === "PERCENTAGE") {
      for (const s of initial.splits)
        map[s.userId] = String(s.sharePercent ?? "");
    }
    return map;
  });
  ...
  const customSum = participantIds.reduce(
    (acc, id) => acc + (parseFloat(customAmounts[id]) || 0),
    0,
  );
  const percentSum = participantIds.reduce(
    (acc, id) => acc + (parseFloat(percents[id]) || 0),
    0,
  );
  ...
  const handleSave = async () => {
    if (!isValid) {
      if (splitType === "CUSTOM")
        setError(
          `Custom amounts add up to ${customSum.toFixed(2)}, they need to total ${amountNum.toFixed(2)}.`,
        );
      else if (splitType === "PERCENTAGE")
        setError(
          `Percentages add up to ${percentSum.toFixed(1)}%, they need to total 100%.`,
        );
      else
        setError(
          "Fill in a description, amount, and at least one participant.",
        );
      return;
    }
    setError("");
    ...
    const splits = participantIds.map((userId) => {
      if (splitType === "CUSTOM")
        return { userId, shareAmount: parseFloat(customAmounts[userId]) || 0 };
      if (splitType === "PERCENTAGE")
        return { userId, sharePercent: parseFloat(percents[userId]) || 0 };
      return { userId };
    });
    ...
    try {
      ...
      if (!navigator.onLine) {
        setError(
          "You're offline. Group expenses need an internet connection - try again once you're back online.",
        );
      } else {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={initial ? "Edit expense" : "Add group expense"}
    >
      <div style={{ display: "grid", gap: 12, marginBottom: 14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", marginTop: 6 }}>
            {paidById === currentUserId ? "You'll" : "They'll"} get the full
            amount logged as a personal expense on the Dashboard right away.
          </div>
        </div>
      </div>
          ...
          {SPLIT_TYPES.map((t) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                background:
                  splitType === t.value ? "var(--surface)" : "transparent",
        ...
        {splitType === "CUSTOM" && (
          <div
            style={{
              ...
              fontSize: 12,
              color:
                Math.abs(customSum - amountNum) < 0.01
                  ? "var(--ink-3)"
                  : "var(--neg)",
            }}
          >
            {getCurrencySymbol("INR")}
            {customSum.toFixed(2)} of {getCurrencySymbol("INR")}
