...
export async function createExpenseRecord(userId, data, client = prisma) {
  ...
  if (data.type === "INCOME") {
    if (data.categoryId) {
      const cat = await client.category.findFirst({
        where: { id: data.categoryId, userId },
      });
      if (!cat) return { error: "Invalid category", statusCode: 400 };
    } else if (data.sourceId) {
      const src = await client.incomeSource.findFirst({
        where: { id: data.sourceId, userId },
      });
      if (!src) return { error: "Invalid income source", statusCode: 400 };
    } else {
      return {
        error: "categoryId or sourceId is required for income",
        statusCode: 400,
      };
    }
  } else {
    if (!data.categoryId)
      return { error: "categoryId is required for expenses", statusCode: 400 };
    const cat = await client.category.findFirst({
      where: { id: data.categoryId, userId },
    });
    if (!cat) return { error: "Invalid category", statusCode: 400 };
  ...
  const expense = await client.expense.create({
    data: {
      ...
      categoryId:
        data.type === "EXPENSE" ? data.categoryId : data.categoryId || null,
      sourceId:
        data.type === "INCOME" && !data.categoryId ? data.sourceId : null,
...
export async function updateExpenseRecord(userId, id, data, expectedUpdatedAt) {
  ...
  if (
    ...
    const serverRecord = await prisma.expense.findUnique({
      where: { id },
      include: EXPENSE_INCLUDE,
    });
...
export async function deleteExpenseRecord(userId, id) {
  ...
  if (existing.goalId) {
    const goal = await prisma.goal.findFirst({
      where: { id: existing.goalId, userId },
    });
    if (goal) {
...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (req.query.search)
      where.note = { contains: req.query.search, mode: "insensitive" };
...
router.get("/export", async (req, res, next) => {
  try {
    ...
    const csv = [
      "Date,Type,Amount,Category,Tags,Note",
