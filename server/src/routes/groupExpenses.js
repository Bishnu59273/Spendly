...
async function findOrCreateSettlementCategory(client, userId) {
  let category = await client.category.findFirst({
    where: { userId, name: SETTLEMENT_LEDGER_NAME },
  });
  if (!category) {
    category = await client.category.create({
      data: {
        name: SETTLEMENT_LEDGER_NAME,
        color: "#1d6b51",
        icon: SETTLEMENT_LEDGER_ICON,
        userId,
...
async function findOrCreateGroupExpenseCategory(client, userId) {
  let category = await client.category.findFirst({
    where: { userId, name: GROUP_EXPENSE_LEDGER_NAME },
  });
  if (!category) {
    category = await client.category.create({
      data: {
        name: GROUP_EXPENSE_LEDGER_NAME,
        color: "#1d6b51",
        icon: GROUP_EXPENSE_LEDGER_ICON,
        userId,
        isSystemManaged: true,
      },
    });
...
function resolveSplits(amount, splitType, splits) {
  ...
  if (splitType === "CUSTOM") {
    const sum = roundMoney(
      splits.reduce((acc, s) => acc + (s.shareAmount ?? 0), 0),
    );
    if (Math.abs(sum - amount) > 0.01) {
      return {
        error: `Custom split amounts (${sum}) must add up to the total (${amount}).`,
        statusCode: 400,
      };
    }
    return {
      splits: splits.map((s) => ({
        userId: s.userId,
        shareAmount: roundMoney(s.shareAmount ?? 0),
        sharePercent: null,
      })),
    };
  }
  ...
  if (Math.abs(pctSum - 100) > 0.5) {
    return {
      error: `Split percentages (${pctSum}%) must add up to 100%.`,
      statusCode: 400,
    };
  }
  const resolved = splits.map((s) => ({
...
export async function createGroupExpenseRecord(userId, groupId, data) {
  const memberErr = await membersError(groupId, [
    data.paidById,
  ...
  const expense = await prisma.$transaction(
    async (tx) => {
      ...
      const category = await findOrCreateGroupExpenseCategory(
        tx,
        data.paidById,
      );
...
router.post("/:groupId/expenses", async (req, res, next) => {
  try {
    ...
    const result = await createGroupExpenseRecord(
      req.userId,
      req.params.groupId,
      data,
    );
...
router.patch("/:groupId/expenses/:id", async (req, res, next) => {
  try {
    ...
    const data = groupExpenseSchema.parse(req.body);
    const result = await updateGroupExpenseRecord(
      req.userId,
      req.params.groupId,
      req.params.id,
      data,
    ...
    if (result.forbidden)
      return res
        ...
        .json({
          error:
            "Only the person who logged this expense or the group owner can edit it",
        });
    if (result.error)
      return res.status(result.statusCode).json({ error: result.error });
...
router.delete("/:groupId/expenses/:id", async (req, res, next) => {
  try {
    ...
    const result = await deleteGroupExpenseRecord(
      req.userId,
      req.params.groupId,
      req.params.id,
    );
...
router.post("/:groupId/settlements", async (req, res, next) => {
  try {
    ...
    if (fromUserId === toUserId) {
      return res
        .status(400)
        .json({ error: "A settlement needs two different people" });
    }
    if (req.userId !== fromUserId && req.userId !== toUserId) {
      return res
        .status(400)
        .json({ error: "You must be one of the two parties in a settlement" });
    }
    const memberErr = await membersError(req.params.groupId, [
      fromUserId,
      toUserId,
    ]);
    if (memberErr)
      return res.status(memberErr.statusCode).json({ error: memberErr.error });
...
router.patch("/:groupId/settlements/:id", async (req, res, next) => {
  try {
    ...
    const { action } = z
      .object({ action: z.enum(["confirm", "decline"]) })
      .parse(req.body);
    ...
    if (settlement.status !== "PENDING")
      return res
        .status(409)
        .json({ error: "This settlement has already been resolved" });

    const otherParty =
      settlement.initiatedById === settlement.fromUserId
        ? settlement.toUserId
    ...
    if (req.userId !== otherParty) {
      return res
        ...
        .json({
          error:
            "Only the other party in this settlement can confirm or decline it",
        });
    }
